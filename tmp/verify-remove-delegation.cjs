...
for(const username of ['calisan','ihtiyac_admin','teknik_admin','muhendislik_admin']){
 ...
 await page.locator('.case-header').waitFor({timeout:15000});if(await page.getByRole('button',{name:'Vekâletlerim',exact:true}).count())throw Error('Removed navigation still visible');await page.evaluate(()=>{state.currentPage='portal-delegations';render()});await page.locator('#operationsContent').waitFor();if(await page.evaluate(()=>state.currentPage)!=='portal-inbox')throw Error('Old route was not redirected');console.log(JSON.stringify({username,opened:true,removedSection:true,oldRouteRedirected:true,errors}));
