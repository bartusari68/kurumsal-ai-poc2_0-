...
base=Path('tmp/faz12-preservation.json');manifest=json.loads(base.read_text())
old=sqlite3.connect('file:backups/faz12-before.db?mode=ro',uri=True);new=sqlite3.connect('file:data/app.db?mode=ro',uri=True)
...
report['new_tables']={t:new.execute('SELECT count(*) FROM '+t).fetchone()[0] for t in ('position_profiles','position_revisions','position_requirements','user_positions','position_audit','request_position_sources')}
report['passed']=not report['changed_business_tables'] and report['pdfs_unchanged'] and not report['foreign_key_violations']
Path('tmp/faz12-preservation-report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
